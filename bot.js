...
client.on('ready', () => {                           
client.user.setGame(` Premium ℕova ♥ | -help `,'https://www.twitch.tv/♥ Premium ℕova ♥');
});
...
if(!message.channel.guild) return;
  ...
  if(!message.member.hasPermission('ADMINISTRATOR')) return message.reply(' **You Dont Have Administrator**');
	  let role = message.guild.roles.find('name', 'Rainbow ℕova')
    if(role) return message.channel.send(`This Step Already Completed !`)//if the role already created return with this msg
  //start of create role 
  if(!role){
    rainbow =  message.guild.createRole({
   name: "Rainbow ℕova",//the role will create name
...
client.on('ready', () => {//new ready event
  setInterval(function(){
      client.guilds.forEach(g => {
                  var role = g.roles.find('name', 'Rainbow ℕova');//rainbow role name
                  if (role) {
